...
When an appliance is downloaded, short hashes may be appended to names to guarantee uniqueness. This applies to sub-objects (VM templates and disks). To make them recognizable, the original sub-object name is preserved and placed before the hash and index. For example:
```default
$ oneimage list
